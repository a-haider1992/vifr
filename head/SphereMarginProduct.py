...
class SphereMarginProduct(nn.Module):
    ...
    def forward(self, input, label):
        ...
        cos_theta = cos_theta.clamp(-1, 1)  # Use .clamp to ensure values are within valid range

        cos_m_theta = self.margin_formula[self.m](cos_theta)  # Corrected: use self.m instead of self.m(theta)
